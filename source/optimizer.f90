...
!    cleve moler, university of new mexico, argonne national lab.
   ...
   integer,intent(out) :: info !!  * `info = 0` for normal return.
                               !!  * `info = k` signals an error condition.  the
                               !leading minor
                               !!     of order `k` is not positive definite.
   real(wp),intent(inout) :: a(lda,*) !! Dimension `(lda, n)`:
                                      !!
                                      !!  * On entry: the symmetric matrix to be
                                      !factored.  only the
                                      !!    diagonal and upper triangle are
                                      !used.
                                      !!  * On return: an upper triangular
                                      !matrix `r` so that `a = trans(r)*r`
                                      !!    where `trans(r)` is the transpose.
                                      !!    the strict lower triangle is
                                      !unaltered.
                                      !!    if `info /= 0`, the factorization is
                                      !not complete.
...
real(wp),intent(in) :: t(ldt,*) !! t contains the matrix of the system. the zero
                                ...
                                !and
                                !! the corresponding elements of the array can
                                !be
                                !! used to store other information.
real(wp),intent(inout) :: b(*) !! On entry: the right hand side of the system.
                               !! On return: the solution, if info == 0.
